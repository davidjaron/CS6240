...
//      FileSystem fs = FileSystem.get(context.getConfiguration());
      FileSystem fs = null;
      String num = context.getConfiguration().get("filename");
      try {
        fs = FileSystem.get(new URI("s3://cs6240-hw4/input" + num),context.getConfiguration());
    ...
    public void map(final LongWritable key, final Text value, final Context context) throws
      ...
      System.out.println("Wow the filesystem works this way");
      Vertex v = new Vertex();
...
//      }
      ...
      initialJobConfiguration.set("filename", String.valueOf(i-1));
...
//      if (i == 0){
//        initialJob.addCacheFile(new Path(args[0] + (i-1) + "/part-r-00000").toUri());
//      } else {
//        initialJob.addCacheFile(new Path(args[0] + (i-1) + "/part-m-00000").toUri());
//      }

      FileSystem fs = null;
      try {
        fs = FileSystem.get(new URI(args[0] + (i-1)),initialConf);
      } catch (URISyntaxException e) {
        e.printStackTrace();
      }
      if (fs != null) {
        FileStatus[] statuses = fs.listStatus(new Path(args[0] + (i-1)));
        for (FileStatus s : statuses){
          if (!s.getPath().getName().contains(".") && !s.getPath().getName().contains("_")){
            initialJob.addCacheFile(s.getPath().toUri());
          }
        }
      }
